...
pub fn bmr_emitter(_attr: TokenStream, item: TokenStream) -> TokenStream {
...
fn expand(i: ImplItemMethod) -> ImplItemMethod {
    ...
    assert!(
        ...
        "#[bmr_emitter] methods should start with `emit_`"
    );
    let block = {
        let node_type = {
            i.sig
                ...
                .expect(
                    "#[bmr_emitter] methods should have signature of
    ...
    ",
        ...
        Quote::new_call_site()
            .quote_with(smart_quote!(
                ...
                {
                    {
                        impl crate::codegen::BoomerNode for NodeType {
                            fn emit_with_bmr<W>(&self, e: &mut crate::codegen::Emitter<'_, W>) -> Result
                            where
                                W: std::io::Write,
                            {
                                e.mtd_name(self)
